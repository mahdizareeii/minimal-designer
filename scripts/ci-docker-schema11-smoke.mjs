...
Builds a disposable Docker Compose project, proves schema-16 readiness,
...
async function main() {
  ...
  const project = `formaspeccischema16${randomBytes(5).toString("hex")}`;
  ...
  const summary = {
    format: "formaspec-docker-schema16-ci-evidence",
    schemaVersion: 1,
    releaseStatus: "NO-GO",
  ...
  try {
    ...
    const created = await requestJson(`${baseUrl}/api/designs`, {
      ...
      body: JSON.stringify({ name: "Schema 16 Docker CI", preset: "web", idempotencyKey: "docker-schema16-ci-create-0001" }),
    });
  ...
  process.stdout.write(`Docker schema-16 smoke passed; retained evidence: ${evidenceDirectory}\n`);
}
