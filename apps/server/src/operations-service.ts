...
function buildPortableIdMapping(
  ...
  const mapping = Object.fromEntries(collectPortableProjectIds(document, productSpecification).map((sourceId) => [
    sourceId,
...
function portableAssetBytes(imported: ImportedPortableProject, assetId: string): Buffer {
...
function portableProductSpecification(imported: ImportedPortableProject): ProductSpecification | null {
...
export class OperationsService {
  ...
  async importPortableProject(
    ...
    const requestHash = hashPayload({ bundleSha256, mode, importSemanticsVersion: 1 });
    ...
    for (const sourceAssetId of normalizedSourceAssetIds) {
      ...
      const normalized = await normalizeImageAsset(
        portableAssetBytes(imported, sourceAssetId),
    ...
    for (const asset of preparedAssets) {
      this.service.assetStore!.writeNormalized(asset.data, asset.mimeType, asset.sha256);
    }
  ...
  async runScheduledBackup(actorId: string, at = new Date()): Promise<ScheduledBackupRunResult> {
    ...
    try {
      ...
      appendAuditEvent(this.service.database.sqlite, access, "backup.schedule_run", "backup", backup.id, {
        dueAt: window.dueAt,
        nextDueAt: window.nextDueAt,
        retentionClass,
      });
  ...
  private publicBackupSchedule(access: AccessContext, at: Date): PublicBackupSchedule {
    const row = this.backupScheduleRow(access);
    const policy = this.backupPolicy(access);
    ...
    ).get(access.organizationId) as { completed_at: string } | undefined;
    return {
      enabled,
      cronExpression,
      timezone: "UTC",
      ...
      nextDueAt: enabled ? backupScheduleWindow(cronExpression, at).nextDueAt : null,
    };
  }
  ...
  private scheduledBackupInWindow(access: AccessContext, dueAt: string, nextDueAt: string): BackupRecordRow | undefined {
    return this.service.database.sqlite.prepare(
      `SELECT * FROM backup_records
       WHERE organization_id = ? AND retention_class <> 'manual' AND created_at >= ? AND created_at < ?
       ORDER BY created_at DESC, id DESC LIMIT 1`,
    ).get(access.organizationId, dueAt, nextDueAt) as BackupRecordRow | undefined;
  }
  ...
  private backupRetentionPlan(access: AccessContext): BackupRetentionPlan {
    ...
    return buildBackupRetentionPlan(records, this.backupPolicy(access).retention);
  }
