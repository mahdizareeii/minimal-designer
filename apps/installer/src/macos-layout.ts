...
reject_runtime() { echo "$1" >>"\${LOG_ROOT}/protocol.log"; exit 1; }
...
case "\${URL}" in
  ...
  formaspec://open-review\?design=*)
    ...
    ACTIVE_STORE="$(printf '%s' "\${ENSURE_JSON}" | /usr/bin/sed -n 's/.*"dataStoreId":"\\([^"]*\\)".*/\\1/p')"
    WEB_ORIGIN="$(printf '%s' "\${ENSURE_JSON}" | /usr/bin/sed -n 's/.*"webOrigin":"\\([^"]*\\)".*/\\1/p')"
