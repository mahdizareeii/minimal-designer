...
function acquireApplicationLock(projectRoot: string): () => void {
  ...
  try {
    ...
    const owner = fs.existsSync(pidFile) ? fs.readFileSync(pidFile, "utf8").trim() : "";
    if (/^\d+$/.test(owner)) {
      try {
...
class DockerRestoreSupervisor {
  ...
  constructor(projectRoot: string, dependencies: DockerRestoreDependencies) {
    ...
    try {
      ...
      throw new Error(
        "The pinned local Docker runtime binding is missing or invalid. Run 'formaspecctl restart' with the current installer before restore.",
        { cause: error },
      );
    }
    this.#runner = dependencies.commandRunner ?? runCommand;
    this.#environment = sanitizedDockerEnvironment(dependencies.environment ?? process.env);
  ...
  async #waitFor(pathname: string, predicate: (status: number, body: unknown) => boolean): Promise<void> {
    const binding = await this.#verifiedBinding();
    const deadline = Date.now() + 120_000;
    let lastStatus = 0;
    while (Date.now() < deadline) {
      try {
        const response = await this.#fetch(`${binding.publicBinding.origin}${pathname}`, {
