...
| Renderer SSRF/data exfiltration | Docker uses a separate worker with `network_mode: none`; request interception and no-remote-URL schemas remain defense in depth. The historical schema-13 Docker canary failed closed with DNS `EAI_AGAIN`, direct TCP `ENETUNREACH`, and zero external interfaces, and workflow contracts require summary evidence. | Rerun the canary at schema 15, retain hosted evidence, then add native-package boundary evidence and real server deployment verification. |
