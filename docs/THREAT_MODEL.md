...
| --- | --- | --- |
| Forged proxy identity | Server mode validates configured proxy ranges, Host, Origin, CSRF intent, and identity presence; generated configs are fail-closed. | Prove the reverse proxy strips caller headers and add the full CIDR/direct-port deployment matrix. |
...
| Unapproved repository modification | Workspace Bridge exposes inventory reads only and has no shell, subprocess, network, or mutation capability. | Add separate plan/approval/diff/validation/commit/PR gates before any handoff implementation; central server remains shell-free. |
...
| Supply-chain compromise | The exact pnpm version and lockfile are pinned. An offline deterministic CycloneDX 1.6 source-workspace SBOM, license/notice evidence, SHA-256 manifest, and fail-closed permissive-only policy gate exist. The Sharp-free source workspace passes with 342 third-party components and zero violations. A fresh unsigned macOS ARM64 candidate has offline package/BOM/payload integrity evidence, source-component linkage, and exact file/type/path/content-hash equality for all seven packaged workspace output trees; stale compiled files fail even when manifests and selected entry points match. Its gate fails closed because Chromium's composite notice contains LGPL text and signature/notarization, reproducibility, and vulnerability-scan evidence are missing. | Resolve the notice-policy blocker; generate artifact evidence for containers, Windows, and Linux; add dependency/image/OS vulnerability scans, independent reproducibility, retained CI evidence, and real provenance/signing/notarization when credentials exist. |
