...
| Renderer SSRF/data exfiltration | Docker uses a separate worker with `network_mode: none`; request interception and no-remote-URL schemas remain defense in depth. The exact-current Docker canary fails closed with DNS `EAI_AGAIN`, direct TCP `ENETUNREACH`, and zero external interfaces, and workflow contracts require its summary evidence. | Retain hosted canary evidence, then add native-package boundary evidence and real server deployment verification. |
