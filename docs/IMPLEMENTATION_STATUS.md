...
The current schema-18 package suites pass 1,058/1,058: core 74, server 551/551,
web 148, CLI 146 (140 ordinary plus six bridge-lifecycle), local bridge 27,
Workspace Bridge 37, and installer 75. Four
...
typecheck and production build pass, launcher tests pass 284/284, and
...
Current source contains the Organization-Ready 0.4.0 delta and is now database
schema 18. It adds first-class Products above Designs, immutable Product-bound
...
recovery, exactly one managed FormaSpec 0.4.0 plugin, linked responsive frame
...
schema-18 package/typecheck/build/launcher gates above pass; full browser,
Docker/recovery, current scan, packaged Codex-plugin lifecycle, and supported-OS
release aggregates remain pending.

The live macOS source-checkout upgrade completed on 2026-07-25. It started
Docker against the recorded data store, built the current image, reached
schema-18 readiness, passed the isolated Playwright renderer check, and matched
the bridge's upstream origin and data-store identity. Strict `doctor` passed an
authenticated MCP `initialize` plus `tools/list` and confirmed all 12 essential
tool IDs. The live Codex inventory contains exactly one
`formaspec@formaspec` plugin at 0.4.0 and one token-free `formaspec` MCP entry,
with no standalone FormaSpec/legacy compatibility skills and no legacy
compatibility marketplace entry or text in the Codex-managed configuration. A
new Codex task is required to load the refreshed plugin inventory. This is
source-checkout workstation evidence, not packaged installer, custom-protocol,
hosted, signed, scanned, or supported-OS release qualification.
...
| Organization-Ready 0.4.0 source delta | **Implemented foundation; release gate open.** Source declares database schema 18, command engine 3, one FormaSpec 0.4.0 plugin, 54 MCP tools, 26 resources, and 119 protected non-MCP routes. Migration 17 introduces Products, deterministic Design-to-Product backfill, Product move previews, and immutable Product-bound agent-task context; migration 18 normalizes and strengthens recoverable archive integrity. Source also implements strict design-readiness evidence, content-addressed exact-preview PNG artifacts, `formaspec://open-review` links, fixed-purpose `ensure-running` recovery, linked responsive frames, richer component materialization, and deterministic sanitized SVG/PDF export. Current package suites pass 1,058/1,058 and recursive typecheck/build pass. The live macOS source-checkout upgrade also passes current-image schema-18 readiness, isolated rendering, bridge origin/store verification, strict MCP doctor, and single-plugin/token-free configuration checks; full browser, retained Docker/recovery release evidence, scans, packaged Codex-plugin lifecycle, and supported-OS qualification remain pending. |
...
| Schema-18 migration fixtures | Deterministic schema 1 and schema 7–12 fixtures preserve stable IDs, canonical bytes, hash chains, and enterprise rows through the ordered migrations. Migration-14/15 fixtures add authentication and nullable exact-render metadata without fabricating an administrator, session, or historical PNG evidence. Migration-16 tests upgrade the exact legacy schema-14 and schema-15 bootstrap trigger, preserve credential rows, install the canonical trigger, and fail closed on schema-shape drift. Migration-17 source and fixtures deterministically create one active Product per existing Design, preserve Design IDs and heads, require a valid owner, and add immutable Product/task integrity. Migration-18 fixtures normalize invalid canonical pointers and strengthen active same-Product integrity. The focused migration/backup/restore cluster passes 59/59; broader anonymized-customer, packaged, and real remote-host restore fixtures remain pending. |
| Current all-workspace typecheck/build | Recursive schema-18 typecheck and production build pass across all seven workspace packages. Neither gate replaces signed artifacts, hosted provenance, current security scans, browser/Docker release evidence, or supported-OS lifecycle proof. |
| Current launcher suite | The schema-18 launcher suite passes 284/284 after session-mode initialization, bootstrap-token handling, pairing-ticket updates, reciprocal native/Docker data-store guards, Docker running/restarting/paused/created-state detection, bridge ownership and upstream-identity validation, mode-aware recovery guidance, transition locking, container-mode host-Node bypass, effective loopback Host comparison, and local/Docker dry-run contracts that execute no package, CLI, Compose, or runtime-state mutation on a Node-less machine. |
...
predates migrations 13–18, exhaustive event authorization, the project/
...
single FormaSpec 0.4.0 plugin. A new package from current source has not been
built or installed.
...
Current runtime metadata is database schema 18, document schema 2, command
...
`apps/cli/src/migrations.ts` recognizes schema 18, matching the server source.
The current schema-18 CLI package passes 146/146, including 140 ordinary CLI
tests and the six bridge-lifecycle cases requiring local process/IPC access.
The current installer package passes 75/75. The live macOS source-checkout
Codex upgrade is verified; packaged supported-OS installer and custom-protocol
...
passed, and current schema-18 package/typecheck/build gates pass, but the full
...
| Product/design/spec/planning/task/connection MCP tools | **Implemented foundation** | Current source exposes 54 tools and 26 resources, adding Product list/read discovery to the strict organization-policy, design/history, exact task-backed preview, product-specification, planning, task, connection, design-system, inventory, mapping, handoff, Redesign Studio, and component-insertion surface. Preview tools return PNG plus immutable render options/dimensions/backend/warnings/SHA-256; exact resources and default `design_render(preview_id)` replay and verify the same evidence. Multi-page changes use one bounded contact sheet; page/token/component scope inference and archived-page tombstones prevent hidden changes. Human-authenticated website routes are the design-preview commit boundary. The current server package passes 551/551, including Product, readiness, task-context, preview-artifact, and review-link coverage. | Add hosted agent release E2E and broader long-running/revocation/load coverage |
| Immutable tasks and append-only transitions | **Implemented foundation** | Persistent inputs, transition history, claim/progress/completion/cancellation/expiry checks, audit/outbox, MCP creation, and durable website Activity/status/review surfaces exist. Migration 17 binds each new task to immutable resolved Product/Design context: Product identity, exact Design head/version, canonical specification hash, effective design-system release, locale, direction, platforms, and repository-inventory hashes. Codex/CLI agents preview, inspect, lint, return the readiness report, and transition to `awaiting_approval`; only the authenticated website action commits or discards the exact preview. Current focused Product/readiness/preview/MCP tests pass 44/44. | Extend expected-output enforcement across every task kind, retain the full browser workflow in release CI, and broaden cancellation/expiry browser coverage |
| Automatic Codex adapter | **Implemented foundation** | `formaspecctl` starts the bridge, consumes an administrator-issued one-time pairing ticket when required, configures one token-free `formaspec` MCP with server-scoped automatic approval, and installs/verifies exactly one managed FormaSpec 0.4.0 plugin before removing only installer-owned duplicate legacy assets. Unmanaged files are preserved and reported. Startup refreshes an already-authorized managed install, including migration from prior `writes` or `never` values to `approve`, while preserving unrelated TOML, global approval, and sandbox policy. Bare, basic-quoted, and literal-quoted approval keys are treated semantically; inline comments and unrelated content are preserved, while semantic duplicates fail before writing. Managed cleanup removes the exact legacy Minimal UI TOML parent/descendants while preserving multiline-string lookalikes, similarly prefixed IDs, unrelated configuration, failure safety, and retry idempotency. The fixed-purpose `formaspecctl ensure-running` command resumes only the recorded local, development, Docker, or server runtime, verifies the exact data-store identity, starts or validates the matching bridge, and refuses to guess or switch modes. `doctor` and `status` compare the UI/API, bridge, known Docker binding, exact `codex mcp get formaspec --json` loopback transport/URL, and managed local approval mode instead of independently accepting different healthy instances. Current CLI tests pass 146/146, local bridge 27/27, Workspace Bridge 37/37, and installer 75/75. A live macOS source-checkout upgrade built and started the current Docker image on the recorded store, passed schema-18 readiness/isolated rendering/bridge identity checks, and passed strict MCP `initialize`/`tools/list` doctor verification. After the effective loopback Host-comparison fix, `./designer ensure-running --json` returns ready for recorded Docker origin/web origin `http://127.0.0.1:4310`, store `store_70354ab57f8b26194138df3e1c443e4b`, and `bridgeReady: true`. Live Codex now has exactly one 0.4.0 managed plugin, one token-free MCP entry, no standalone skills, and no legacy compatibility marketplace/configuration residue; a new task is required to load that inventory. | Complete the clean packaged install/upgrade/reconnect/revoke and custom-protocol matrix on supported OSes and packaged runtimes |
...
| Requirement | Status | Evidence | Remaining work |
| --- | --- | --- | --- |
| Current installer source and protocol gate | **Implemented foundation** | The current schema-18 installer suite passes 75/75, recursive typecheck/build passes, and the macOS packaged-runtime contract passes 11/11. Current source packages and validates exactly one FormaSpec 0.4.0 plugin, rejects forbidden retired package paths, and removes installer-owned legacy assets only after the new plugin verifies. Ticket-aware protocol handlers for macOS, Linux, and Windows retain a 512-character URL limit, exact nonce/connection formats, fixed executable paths, quoted arguments, and strict malformed/credential-bearing input rejection. The canonical secret-free `formaspec://open-review?design=…&preview=…&task=…&store=…` handler invokes `formaspecctl ensure-running --json`, validates the exact origin/store identity, and opens the persisted review route. The live macOS source-checkout upgrade verifies the current single-plugin/token-free configuration path, but it does not qualify a packaged installer or registered protocol handler. | Build and install frozen artifacts on supported operating systems; prove packaged Codex upgrade, protocol registration, upgrade/uninstall/reinstall, signing, ACL/service isolation, and end-to-end handler lifecycle. |
...
| Release CI and evidence | **Partial** | Current schema-18 package suites pass 1,058/1,058 (core 74, server 551, web 148, CLI 146, local bridge 27, Workspace Bridge 37, installer 75), and recursive typecheck/build pass. Launcher 284/284, Compose configuration, deterministic export 15/15, migration/backup/restore 59/59, Product/readiness/preview/MCP 44/44, macOS packaged-runtime contracts 11/11, release-evidence contracts 8/8, CI workflow contracts 8/8, off-host contracts 7/7, and cross-browser Docker contracts 2/2 pass. A live macOS source-checkout upgrade additionally passes the current-image schema-18 runtime, isolated renderer, bridge identity, strict MCP doctor, single-plugin/token-free Codex checks, and exact `ensure-running` recovery after the effective loopback Host-comparison fix. Retained schema-16 browser/runtime results remain regression evidence, not schema-18 release provenance. Current full browser, retained Docker/recovery release runs, scans, packaged Codex-plugin lifecycle, and supported-OS release evidence remain pending. Exact 342-component/zero-violation SBOM/license evidence remains historical schema 13. | Run and retain the remaining schema-18 gates, freeze provenance-bound artifacts, run current scans, and complete signed native delivery, supported-OS lifecycle, hosted, and real remote-recovery evidence. |
...
- Retain the passing schema-18 package/typecheck/build gates and run the
  remaining provenance-bound launcher, full browser, Docker, recovery, and
  packaged Codex-plugin aggregates under release CI. The live macOS
  source-checkout upgrade is operational evidence only. Retained schema-16 launcher,
...
- A release-ready native installer set. The unsigned schema-12 macOS ARM64 PKG
  ...
  runtime checks, and was not installed. It predates schemas 13–18, component
  source persistence/insertion, Products, immutable Product-bound tasks, the
  single FormaSpec 0.4.0 plugin, and the current 54-tool/26-resource/119-route
