...
suite at 920/920: core 64, server 496, web 126, CLI 103, local bridge 22,
Workspace Bridge 37, and installer 72. Launcher passes 227/227. Current browser
and integration evidence passes editor/Administration 11/11, the release scenario
...
| Current schema-16 application, authentication, and exact-preview gates | Source declares database schema 16. Migrations 14 and 15 add browser-session authentication and bounded write-once exact preview-render metadata; migration 16 canonicalizes the legacy bootstrap-credential consumption trigger. The complete seven-package suite passes 920/920: core 64, server 496, web 126, CLI 103, local bridge 22, Workspace Bridge 37, installer 72. Launcher 227/227, editor/Administration E2E 11/11, release scenario 1/1, Chromium alignment, visual regression 7/7, macOS runtime contracts 11/11, Docker Firefox/WebKit 12/12, CI workflow contracts 8/8, and off-host contract tests 7/7 pass. |
...
| Launcher suite | 227/227 passed after session-mode initialization, bootstrap-token handling, pairing-ticket updates, and local/Docker dry-run contracts that execute no package, CLI, Compose, or runtime-state mutation on a Node-less machine. |
...
| Automatic Codex adapter | **Implemented foundation** | `formaspecctl` starts the bridge, consumes an administrator-issued one-time pairing ticket when required, configures one token-free `formaspec` MCP with write approvals, and installs/verifies the primary managed version-0.2.1 FormaSpec skills/plugins plus the Minimal UI compatibility assets. Startup refreshes an already-authorized managed install. The bridge state pins the exact upstream MCP URL and restarts when the API port changes; authenticated verification accepts bounded enterprise `tools/list` responses up to 1 MiB and still rejects oversized responses. Local `doctor` passes server, renderer, bridge, MCP initialize, and 12-essential-tool checks. | Clean install/upgrade/reconnect/revoke matrix on supported OSes and packaged runtime |
...
| Requirement | Status | Evidence | Remaining work |
| --- | --- | --- | --- |
| Loopback agent bridge | **Implemented foundation** | A bounded local bridge proxies MCP and stores only the scoped upstream grant outside Codex configuration. Current tests pass 22/22 across two files, including authenticated pairing tickets, expected-connection checks, token-free MCP, stored-grant reuse, exact upstream-port restart, bounded large enterprise tool inventories, sanitized control diagnostics, and fail-closed behavior. | Package as a self-contained pinned-runtime executable and complete installed cross-platform credential/protocol lifecycle proof |
