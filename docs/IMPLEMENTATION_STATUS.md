...
The current schema-16 seven-package `test:run` passes 957/957 across 147 test
files: core 64, server 508, web 135, CLI 114, local bridge 27, Workspace Bridge
37, and installer 72. Recursive typecheck and production build pass, as does
the launcher suite at 276/276. Retained browser and integration
...
initialize call returns the primary
`[@FormaSpec](plugin://formaspec@formaspec)` mention and the explicitly labeled
legacy `[@Minimal UI](plugin://minimal-ui@formaspec)` compatibility alias
inside the bounded 510-character
server instructions.
...
| Current schema-16 application, authentication, and exact-preview gates | Source declares database schema 16 and command engine 3. Migrations 14 and 15 add browser-session authentication and bounded write-once exact preview-render metadata; migration 16 canonicalizes the legacy bootstrap-credential consumption trigger. The current seven-package suite passes 957/957 across 147 files: core 64, server 508, web 135, CLI 114, local bridge 27, Workspace Bridge 37, and installer 72. Recursive typecheck/build and launcher 276/276 pass. Retained editor/Administration E2E 11/11, release scenario 1/1, Chromium alignment, visual regression 7/7, macOS runtime contracts 11/11, Docker Firefox/WebKit 12/12, CI workflow contracts 8/8, and off-host contract tests 7/7 pass. |
...
The current CLI package passes 114/114. Focused reruns also pass CLI command
44/44, generic MCP configuration 4/4, and bridge lifecycle 6/6.
...
| Requirement | Status | Evidence | Remaining work |
| --- | --- | --- | --- |
| FormaSpec MCP identity and workflow | **Implemented foundation** | Server ID `formaspec`, `formaspec://` resources, and canonical/plugin skills require explicit project resolution, a claimed direct-request `design_preview` task, preview/render/lint inspection, and transition to `awaiting_approval`. Agents never call the compatibility commit tool; the exact review image, task/project/base/expiry details, and `reviewDeepLink` are returned for website approval. The token-free MCP is exposed through the primary `[@FormaSpec](plugin://formaspec@formaspec)` identity; the managed `[@Minimal UI](plugin://minimal-ui@formaspec)` identity remains the compatibility alias. | Broaden revocation/reconnection plus large-history cursor/load evidence, retain hosted client runs, and prove supported-OS installed plugin lifecycle |
| Design/product-spec/planning/task/connection MCP tools | **Implemented foundation** | Strict tools cover organization policy, design discovery/history, exact task-backed previews, product specs, planning, tasks, connections, design-system releases/pins/upgrades, revision-bound historical releases, path-free inventories, handoffs, Redesign Studio, and exact pinned-release component insertion. Preview tools return PNG plus immutable render options/dimensions/backend/warnings/SHA-256; exact resources and default `design_render(preview_id)` replay and verify the same evidence. Multi-page changes use one bounded contact sheet; page/token/component scope inference and archived-page tombstones prevent hidden changes. Human-authenticated website routes are the design-preview commit boundary. The current server package passes 508/508, including stable authorization-bound design-list pagination and archived-context opacity. | Add hosted agent release E2E and broader long-running/revocation/load coverage |
| Immutable tasks and append-only transitions | **Implemented foundation** | Persistent inputs, transition history, claim/progress/completion/cancellation/expiry checks, audit/outbox, API/MCP, initial UI actions, and the integrated website-to-agent path exist. Website-task agents preview, inspect, lint, and transition to `awaiting_approval`; the authenticated human website action commits or discards the exact preview. | Expected-output enforcement across every task kind and broader cancellation/expiry browser coverage |
| Automatic Codex adapter | **Implemented foundation** | `formaspecctl` starts the bridge, consumes an administrator-issued one-time pairing ticket when required, configures one token-free `formaspec` MCP with write approvals, and installs/verifies the primary managed version-0.2.2 FormaSpec skills/plugins plus the Minimal UI compatibility assets. Startup refreshes an already-authorized managed install. The bridge state pins the exact upstream MCP URL and stable data-store identity, restarts when the API port changes, and exposes the verified upstream origin/store through health and authorization checks. `doctor` and `status` compare the UI/API, bridge, and known Docker binding instead of independently accepting different healthy instances. The launcher refuses local/dev startup while this workspace's Docker runtime is live, because those modes use a separate data directory. | Clean install/upgrade/reconnect/revoke matrix on supported OSes and packaged runtime |
...
| Release CI and evidence | **Partial** | The current seven-package suite passes 957/957 across 147 files (core 64, server 508, web 135, CLI 114, local bridge 27, Workspace Bridge 37, installer 72); recursive typecheck/build and launcher 276/276 pass. Retained editor/Administration E2E 11/11, release scenario 1/1, Chromium alignment, visual regression 7/7, the 1,000-node budgets, macOS runtime contracts 11/11, Docker Firefox/WebKit 12/12, CI workflow contracts 8/8, and off-host contracts 7/7 pass. Schema-16 Docker restart/egress and same-host clean-target recovery summaries pass. Exact 342-component/zero-violation SBOM/license evidence remains historical schema 13. No signed current native installer, trusted hosted provenance, current dependency/SBOM/image/OS scans, supported-OS lifecycle matrix, or real remote-host/TLS recovery has been retained. | Freeze provenance-bound release artifacts, run current scans, and complete signed native-delivery, supported-OS lifecycle, hosted, and real remote-recovery evidence. |
