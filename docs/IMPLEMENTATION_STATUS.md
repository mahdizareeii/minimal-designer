...
| Requirement | Status | Evidence | Remaining work |
| --- | --- | --- | --- |
| FormaSpec MCP identity and workflow | **Implemented foundation** | Server ID `formaspec`, `formaspec://` resources, preview-inspect-lint plus write-approved direct-commit instructions, and website-task preview/lint/`awaiting_approval` instructions exist alongside strict 52-tool/25-resource contracts with exact nested success DTOs, bounded checksummed `handoff_list` keyset pagination, and the current schema-16 release/preview/task acceptance. The token-free MCP is exposed through the primary `[@FormaSpec](plugin://formaspec@formaspec)` identity; the managed `[@Minimal UI](plugin://minimal-ui@formaspec)` identity is retained only as a legacy compatibility alias. | Broaden revocation/reconnection plus large-history cursor/load evidence, retain hosted client runs, and prove supported-OS installed plugin lifecycle |
...
| Automatic Codex adapter | **Implemented foundation** | `formaspecctl` starts the bridge, consumes an administrator-issued one-time pairing ticket, configures one token-free `formaspec` MCP with write approvals, and installs/verifies the primary managed version-0.2.0 FormaSpec skills/plugins plus the Minimal UI legacy compatibility assets. Codex exposes `[@FormaSpec](plugin://formaspec@formaspec)` as the recommended identity; the `[@Minimal UI](plugin://minimal-ui@formaspec)` alias remains available for existing prompts against the same projects and scoped connection. A new Codex task loads refreshed plugin assets. The bridge fails closed without its OS-stored scoped grant. In authenticated mode it may reuse an exact still-valid grant but cannot create or rotate a connection headlessly. | Clean install/upgrade/reconnect/revoke matrix on supported OSes and packaged runtime |
